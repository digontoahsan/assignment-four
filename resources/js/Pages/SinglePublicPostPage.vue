...
const query = computed(() => {
    ...
    return { from: urlParams.get('from') || 'public' };
});

const backLink = computed(() => query.value.from === 'all' ? '/all-posts' : '/public-posts');
...
<template>
    ...
    <div class="container mt-5 pt-4">
        <Link :href="backLink" class="btn btn-outline-primary mb-4">
            <i class="fas fa-arrow-left me-1"></i> Back to {{ query.from === 'all' ? 'My Posts' : 'Public Posts' }}
        </Link>
    </div>
    <div class="container">
        <!-- Post -->
