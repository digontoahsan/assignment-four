...
class PostController extends Controller
{
    ...
    public function showAllPosts(Request $request)
    {
        ...
        $posts = $query->get(['id', 'title', 'image', 'visibility']);
    ...
    public function showPrivatePosts(Request $request)
    {
        $userId = $request->header('id');
        ...
        $posts = $query->paginate(12)->through(function ($post) use ($userId) {
            return [
                'id' => $post->id,
                'title' => $post->title,
                ...
                'is_bookmarked' => Bookmark::where('user_id', $post->user_id)
                    ->where('post_id', $post->id)
                    ->exists(),
            ];
        });

        return inertia('PrivatePostPage', [
            'posts' => $posts,
        ]);
    }

    public function ShowSinglePrivatePost($id, Request $request)
    {
        $userId = $request->header('id');
        ...
        return inertia('SinglePrivatePostPage', [
            'post' => $post,
        ]);
    }
}
